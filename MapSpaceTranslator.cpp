...
#include"include/TSToOSConverter.h"
...
void NormalGen::prepareOSMap() {
	...
	objectSpaceMap.colour->texWidth = tangentSpaceMap.colour->texWidth;
	objectSpaceMap.colour->texHeight = tangentSpaceMap.colour->texHeight;
	...
	fbufCreateInfo.attachmentCount = 1;
	fbufCreateInfo.pAttachments = &objectSpaceMap.colour->textureImageView;
...
void NormalGen::createOSPipeline() {
	shaderData* sD = new TSTOOSCONVERTERSHADER;
	...
	pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
	pipelineLayoutInfo.setLayoutCount = 1;
	pipelineLayoutInfo.pSetLayouts = &objectSpaceMap.descriptorSetLayout;
...
void NormalGen::prepareOSDescriptor() {
	...
	if (vkCreateDescriptorSetLayout(Engine::get()->device, &layoutInfo, nullptr, &objectSpaceMap.descriptorSetLayout) != VK_SUCCESS) {
		throw runtime_error("failed to create descriptor set layout!");
	}
	...
	if (vkCreateDescriptorPool(Engine::get()->device, &poolInfo, nullptr, &objectSpaceMap.descriptorPool) != VK_SUCCESS) {
		throw runtime_error("failed to create descriptor pool!");
	}

	VkDescriptorSetAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = objectSpaceMap.descriptorPool;
	allocInfo.descriptorSetCount = 1;
	allocInfo.pSetLayouts = &objectSpaceMap.descriptorSetLayout;

	if (vkAllocateDescriptorSets(Engine::get()->device, &allocInfo, &objectSpaceMap.descriptorSet) != VK_SUCCESS) {
		throw runtime_error("failed to allocate descriptor sets!");
	}

	VkDescriptorImageInfo imageInfo{};
	imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	imageInfo.imageView = tangentSpaceMap.colour->textureImageView;
	imageInfo.sampler = Engine::get()->textureSampler;

	VkWriteDescriptorSet descriptorWrite{};
	descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	descriptorWrite.dstSet = objectSpaceMap.descriptorSet;
	descriptorWrite.dstBinding = 0;
	descriptorWrite.dstArrayElement = 0;
	descriptorWrite.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	descriptorWrite.descriptorCount = 1;
