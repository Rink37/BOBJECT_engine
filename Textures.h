...
struct Texture {
  ...
  // I use a mix of OpenCV images and Vulkan images for various reasons, so it
  // makes sense to have a structure which manages conversions
  ...
  VkImageUsageFlags textureUsage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
  ...
  void transitionImageLayout(VkImage, VkFormat, VkImageLayout, VkImageLayout,
                             uint32_t); // We need to be able to explicitly pass
                                        // an image for image copying functions
  ...
  Texture *copyImage(VkFormat, VkImageLayout, VkImageUsageFlags, VkImageTiling,
                     VkMemoryPropertyFlags, uint32_t);
  Texture *copyTexture(VkFormat, VkImageLayout, VkImageUsageFlags,
                       VkImageTiling, uint32_t);
  ...
  virtual void setup() {
    ...
    transitionImageLayout(textureImage, textureFormat,
...
class imageTexture : public Texture {
  // A specific class of texture which constructs an engine image using pixel
  // data OpenCV matrixes are not constructed by default if construction
  // arguments are not OpenCV images, but they can be constructed later - this
  // is just to save memory If the image is constructed using an OpenCV matrix
  // the matrix is stored since we assume the matrix will be used again If it
  // won't be, we can destroy the matrix using destroyCVMat();
public:
  ...
  imageTexture(cv::Mat initMat, VkFormat format, VkImageLayout layout,
               VkImageUsageFlags usage, VkImageTiling tiling, uint32_t ml) {
...
public:
  static webcamTexture *get() {
    if (nullptr == winstance)
      winstance = new webcamTexture;
    return winstance;
  }
  ...
  void getCVMat() { texMat = webCam->webcamFrame; }

  void changeFormat(VkFormat newFormat) { textureFormat = newFormat; }
